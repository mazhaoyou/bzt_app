		...
		<view v-if="maskShow || insert" :class="{ 'ani-calendar-show': aniMaskShow, 'uni-calendar__static': insert }" class="uni-calendar__box">
			...
			<view class="uni-calendar__wrapper">
				<view class="uni-calenda__content">
					...
					<swiper :class="fold ? 'uni-swiper-f-h' : 'uni-swiper-h'" :circular="true" @animationfinish="itemChange" :current="swiperCurrent" :duration="duration">
						<swiper-item v-for="(item, index) in canlenders" :key="index">
							<uni-calendar-item :fold="fold" :disableFold="disableFold" :canlender="item" :lunar="isLunar" @selectDays="selectDays" />
						</swiper-item>
					</swiper>
				</view>
			</view>
		</view>
		<view v-if="!disableFold" class="text-center" @click="clickFlod">
			<image :class="fold ? 'arrow-down' : 'arrow-up'" mode="aspectFit" src="/static/arrowDown.png"></image>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		data() {
			 ...
			 */
			return {
				duration: 500, // swiper 滑动时间
				lastCurrent: 1, // 记录上次swiper current 用来判断滑动方向
				swiperCurrent: 1, //swiper 当前current
				fold: true, // 是否折叠
				currentWeeks: [], // 当前日历周列表
				week: [], // 当前周 [{}]
				selectDay: null, // 选中日期
				maskShow: false, // 显示日历
				aniMaskShow: false,
		...
		methods: {
			...
			getMonthAll(index, date) {
				...
				var canlender2 = {}
				var canlender3 = {}
				if (this.fold && !this.disableFold) {
					// 折叠状态-获取周数据
					canlender1 = this.getDays(this.getDate(date, index - 1, 'week'))
					canlender2 = this.getDays(this.getDate(date, index, 'week'))
					canlender3 = this.getDays(this.getDate(date, index + 1, 'week'))
				}
				if (!this.fold || this.disableFold) {
					canlender1 = this.getWeek(this.getDate(date, index - 1, 'month'))
					canlender2 = this.getWeek(this.getDate(date, index, 'month'))
					canlender3 = this.getWeek(this.getDate(date, index + 1, 'month'))
				}
				if (this.swiperCurrent === 0) {
					this.canlenders = [canlender2, canlender3, canlender1]
				}
				if (this.swiperCurrent === 1) {
					this.canlenders = [canlender1, canlender2, canlender3]
				}
				if (this.swiperCurrent === 2) {
					this.canlenders = [canlender3, canlender1, canlender2]
				}
				this.canlender = canlender2
				if (this.insert) {
					this.setEmit('change')
				}
			},
			...
			getWeek(dateData) {
				...
				let canlender = []
				var afterMonth = month - 1 == 0 ? 12 : month - 1
				var afterYear = month - 1 == 0 ? year-1 : year
				let dates = {
					firstDay: new Date(afterYear, afterMonth, 1).getDay(),
					lastMonthDays: [], // 上个月末尾几天
					currentMonthDys: [], // 本月天数
					nextMonthDays: [], // 下个月开始几天
					endDay: new Date(afterYear, month, 0).getDay(),
					weeks: []
				}
				// 循环上个月末尾几天添加到数组
				for (let i = dates.firstDay; i > 0; i--) {
					const beforeDate = new Date(afterYear, afterMonth, -i + 1).getDate() + ''
					dates.lastMonthDays.push({
						date: beforeDate,
						month: afterMonth,
						disable: this.isDisableBefore(afterYear, afterMonth, beforeDate),
						lunar: this.getlunar(afterYear, afterMonth, beforeDate),
						isDay: false
					})
				}
				let clockinfo = {
				...
				for (let i = 1; i < surplus + 1; i++) {
					dates.nextMonthDays.push({
						date: i + '',
						month: (month + 1) % 12,
						disable: this.isDisableBefore(nextMonthYear, month + 1, i + ''),
						lunar: this.getlunar(nextMonthYear, month + 1, i + ''),
						isDay: false
					})
				}
				...
				// var weeks = this.foldAction(dates.weeks)
				var weeks = dates.weeks
				this.currentWeeks = dates.weeks
				return {
					weeks: weeks,
					month: month,
					date: date,
			...
			getDays(dateData) {
				...
				var weeks = this.foldAction(dates.weeks, dateData)
				this.currentWeeks = dates.weeks
				return {
					weeks: weeks,
					month: month,
			...
			/**
			 * 折叠-数据处理方法-获取周数据 return weeks
			 * */
			foldAction (data, dateData) {
				var _this = this
				const {
					year,
					month,
				...
				var weeks = data
				weeks = data.filter((d, index) => {
					for (var i = 0; i < d.length; i ++) {
						if (d[i]['lunar'] === lunar['IDayCn'] && d[i]['date'] == date && d[i]['month'] == month) {
							return true
						}
					}
				})
			...
			itemChange (event) {
				...
				if ((current - this.lastCurrent) === 1 || (current - this.lastCurrent) === -2) {
					// 向左滑动
					this.duration = 0
					this.dataBefor(1,'month')
